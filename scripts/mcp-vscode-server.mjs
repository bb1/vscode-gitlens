...
server.tool(
	...
	'Launch VS Code with an extension loaded for inspection. Starts a persistent session.',
	{
		...
		session: z.string().optional().describe('Name a persisted VS Code session. Omit for a throwaway session.'),
	},
	async args => {
		if (state === 'ready') {
		...
		try {
			...
			if (sessionDir != null) {
				// Store extension secrets in the persisted session rather than an unavailable OS keyring.
...
server.tool(
	'rebuild_and_reload',
	'Run the build command, then restart the extension host. For webview-only changes, build webviews and use a view refresh command instead of restarting the extension host. NOTE: the evaluator bridge does not survive the restart (it is the --extensionTestsPath entry, which only runs at workbench startup), so evaluate and other bridge-backed tools stop working afterwards — teardown + launch to get them back.',
	{
		build_command: z
			.string()
			.optional()
